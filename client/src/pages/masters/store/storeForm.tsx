...
export default function StoreForm({
  ...
  useEffect(() => {
    if (mode === "edit" || mode === "view") {
      ...
      form.reset({
        franchise_name: initialValues?.franchise_name ?? "",
          ...
          <Box className="flex gap-3">
            <Box w="33%">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel style={{ color: "#000" }}>Franchise Name<RequiredMark show={!isView} /></FormLabel>
                    <FormControl>
                      <Input {...field} placeholder="Enter franchise name" disabled={isView} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
